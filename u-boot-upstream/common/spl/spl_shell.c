...
// ------------------------------------------------------------------ 
static const char shell_invalidArguments[] = "** invalid arguments detected.**\n";
...
typedef struct
{
    ShellExternalCommand shellCommandEntry[SHELL_COMMAND_MAXENTRY];
    bool initialized;
    bool enabled;
    bool cancelEncountered;
    bool exit_tinyshell;
    char command[sizeof(log)-1+SHELL_COMMAND_BUFSIZE];
    char history[SHELL_COMMAND_BUFSIZE*SHELL_COMMAND_HISTORY];
    uint32_t dump_startAddress;
    uint32_t dump_endAddress;
    uint32_t dump4_startAddress;
    uint32_t dump4_endAddress;
    uint32_t edit_startAddress;
}ShellData;

#define	gd_shellCommandEntry	(((ShellData *)(gd->spl_shell_data))->shellCommandEntry)
#define	gd_initialized		(((ShellData *)(gd->spl_shell_data))->initialized)
#define	gd_enabled		(((ShellData *)(gd->spl_shell_data))->enabled)
#define	gd_cancelEncountered	(((ShellData *)(gd->spl_shell_data))->cancelEncountered)
#define	gd_exit_tinyshell	(((ShellData *)(gd->spl_shell_data))->exit_tinyshell)
#define	gd_command		(((ShellData *)(gd->spl_shell_data))->command)
#define	gd_history		(((ShellData *)(gd->spl_shell_data))->history)
#define	gd_dump_startAddress	(((ShellData *)(gd->spl_shell_data))->dump_startAddress)
#define	gd_dump_endAddress	(((ShellData *)(gd->spl_shell_data))->dump_endAddress)
#define	gd_dump4_startAddress	(((ShellData *)(gd->spl_shell_data))->dump4_startAddress)
#define	gd_dump4_endAddress	(((ShellData *)(gd->spl_shell_data))->dump4_endAddress)
#define	gd_edit_startAddress	(((ShellData *)(gd->spl_shell_data))->edit_startAddress)
...
static void shellicptf( const char* arguments );
#endif
...
uint32_t spl_shell_init(void)
{
	...
	gd_initialized = false;
	gd_enabled = false;
	gd_exit_tinyshell = false;
	gd_dump_startAddress = 0;
	gd_dump_endAddress = 0xFF;
	gd_dump4_startAddress = 0;
	gd_dump4_endAddress = 0xFF;
	gd_edit_startAddress = 0;
...
#ifdef CONFIG_SPL_KERMIT_SUPPORT
	shell_addCommand( "loadb",		"load binary by kermit.(arg:loadb <addr> [addr2])",	shellloadb  		);
	shell_addCommand( "go",		    "jump to address without return.(arg:go <addr>)",	    shellgo  		    );
#endif
#ifdef CONFIG_SPL_ICPTF_SUPPORT
	shell_addCommand( "icptf",		"cp img to flash.(arg:icptf <1=spi|2=nand> <addr>)",	shellicptf		);
#endif
#ifdef CONFIG_SPL_DDRSCAN_SUPPORT
...
{
    shell_puts( prompt );
    Shell_Status result = shell_scanf_history( &gd_command[sizeof(log)-1], SHELL_COMMAND_BUFSIZE, gd_history, SHELL_COMMAND_HISTORY );

    if( ( SHELL_OK == result ) && ( '\0' != gd_command[sizeof(log)-1] ) )
    {
        shell_puts( "\n" );

        memcpy( gd_command, log, sizeof(log)-1 );

        shellExecCommand( &gd_command[sizeof(log)-1] );
    }
...
Shell_Status shell_addCommand( const char* command, const char* help, void (*func)( const char* arguments ) )
{
	...
	{
		if( NULL == gd_shellCommandEntry[entry].func )
		{
			if( NULL == empty )
			{
				empty = &gd_shellCommandEntry[entry];
			}

			continue;
		}

		target = gd_shellCommandEntry[entry].command;
...
static void shellExecCommand( const char* command )
{
	...
	(gd_shellCommandEntry[entry].func)( ( NO_ARGUMENT == matching ) ? NULL : &command[current+1] );
}
...
static void shellShowHelp( const char* arguments )
{
	...
	for( entry=0; entry<SHELL_COMMAND_MAXENTRY; entry++ )
	{
		if( NULL != gd_shellCommandEntry[entry].func )
		{
			shell_nprintf( 80," %-24s: %s\n",gd_shellCommandEntry[entry].command, gd_shellCommandEntry[entry].help );
		}
	}
}
...
static void shellDumpMemory_sub( const size_t numberOfArguments, const uint32_t* const argumentList )
{
	...
	{
		case 0:	
			gd_dump_endAddress	 = gd_dump_startAddress+0xFF;	
			break;

		case 1:	
			gd_dump_startAddress = argumentList[START_ADDRESS];
			gd_dump_endAddress	 = gd_dump_startAddress+0xFF;	
			break;
		...
		case 2:	
			...
			gd_dump_startAddress = argumentList[START_ADDRESS];
			gd_dump_endAddress	 = argumentList[END_ADDRESS];
			break;
	...
	for( currentAddress=(volatile char*)(gd_dump_startAddress&(~0xF)); currentAddress<=(volatile char*)gd_dump_endAddress; currentAddress+=16 )
	{
		...
		{
			if( ( currentAddress < (volatile char*)gd_dump_startAddress ) ||
				( currentAddress > (volatile char*)gd_dump_endAddress	) )
			{
	...
	gd_dump_startAddress=gd_dump_endAddress+1;
}
...
static bool shellDumpMemory_sub4( const size_t numberOfArguments, const uint32_t* const argumentList )
{
	...
	{
		case 0:	
			gd_dump4_endAddress	 = gd_dump4_startAddress+0xFF;	
			break;

		case 1:	
			gd_dump4_startAddress = argumentList[START_ADDRESS]&(~3);
			gd_dump4_endAddress	 = gd_dump4_startAddress+0xFF;	
			break;
		...
		case 2:	
			...
			gd_dump4_startAddress = argumentList[START_ADDRESS]&(~3);
			gd_dump4_endAddress	 = argumentList[END_ADDRESS]&(~3);
			break;
	...
	for( currentAddress=(volatile unsigned int*)(gd_dump4_startAddress&(~0xF)); currentAddress<=(volatile unsigned int*)gd_dump4_endAddress; currentAddress+=4 )
	{
		shell_snprintf( result, 12, "%08p:  ",currentAddress );
		current=result+11;
		...
		{
			if( ( currentAddress < (volatile unsigned int*)gd_dump4_startAddress ) ||
				( currentAddress > (volatile unsigned int*)gd_dump4_endAddress	) )
			{
	...
	gd_dump4_startAddress=gd_dump4_endAddress+1;
...
static void shellEditMemory_sub( const size_t numberOfArguments, const uint32_t* const argumentList )
{
	...
	switch( numberOfArguments )
	{
		...
		case 1:	
			gd_edit_startAddress = argumentList[START_ADDRESS];
			break;
...

	{
		...
		currentAddress=(volatile uint8_t*)((gd_edit_startAddress/unit)*unit);
...
nextAddress:
	...
	gd_edit_startAddress=(uint32_t)currentAddress;
}
...
static void shellloadb( const char* arguments )
{
	...
	printf("load binary to address 0x%08X",argumentList[ADDRESS]);
	if( 1 == numberOfArguments )
	{
		argumentList[ADDRESS_COPY] = 0;
	}
	else
	{
		printf(" and copy it to address 0x%08X",argumentList[ADDRESS_COPY]);
	}
	printf("\n");

	spl_kermit_load_binary(argumentList[ADDRESS], argumentList[ADDRESS_COPY]);
}
...
static void shellicptf( const char* arguments )
{
	...
	printf("save uImage to Flash #%d from address 0x%08X\n", argumentList[FLASH], argumentList[ADDRESS]);

	spl_icptf( argumentList[FLASH], (void*)argumentList[ADDRESS] );
}
#endif
...
static bool shellGetCtrlC( void )
{
	return( gd_cancelEncountered );
}

#if 0
static void shellSetCtrlC( void )
{
	gd_cancelEncountered = true;
}
#endif
