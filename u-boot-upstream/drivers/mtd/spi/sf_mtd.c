...
/*
 * Global variables can be read-only in SPL
 * when they reside in Flash
 */
...
typedef struct
{
	struct mtd_info sf_mtd_info;
	bool sf_mtd_registered;
	char sf_mtd_name[8];
} sf_mtd_data_t;
#define	sf_mtd_info		(((sf_mtd_data_t *)(gd->sf_mtd_data))->sf_mtd_info)
#define	sf_mtd_registered	(((sf_mtd_data_t *)(gd->sf_mtd_data))->sf_mtd_registered)
#define	sf_mtd_name		(((sf_mtd_data_t *)(gd->sf_mtd_data))->sf_mtd_name)
...
int spi_flash_mtd_register(struct spi_flash *flash)
{
	...
	if (gd->sf_mtd_data == NULL) {
		gd->sf_mtd_data = malloc(sizeof(sf_mtd_data_t));
		if (!gd->sf_mtd_data) {
			printf("spi_flash_mtd_register: malloc failed\n");
			return -ENOMEM;
		}
		memset(gd->sf_mtd_data, '\0', sizeof(sf_mtd_data_t));
	}
#endif
